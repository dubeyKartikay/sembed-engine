...
std::shared_ptr<HDVector> FileDataSet::getHDVecByIndex(const int &index) {
  ...
  m_file.read(reinterpret_cast<char *>(vector->getDataPointer()),
              dimentions * sizeof(float));
  return vector;
}

std::unique_ptr<std::vector<std::shared_ptr<HDVector>>>
FileDataSet::getNHDVectorsFromIndex(const int &index, const int &n) {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    std::copy_n(buffer + i * dimentions, dimentions,
